...
const adventureBooks: Record<string, AdventureBook> = {
  'forest-spirit': forestSpiritData,
  'tide-of-honor': tideOfHonorData,
  ...
  const processContent = (content: string) => {
    return content.split('\n\n').map((paragraph, index) => {
      if (paragraph.startsWith('## ')) {
        return (
          <h2 key={index} className="text-2xl font-bold mt-8 mb-4 text-red-900">
            {paragraph.replace('## ', '')}
          </h2>
        );
      } else if (paragraph.startsWith('### ')) {
        return (
          <h3 key={index} className="text-xl font-semibold mt-6 mb-3 text-red-800">
            {paragraph.replace('### ', '')}
          </h3>
        );
      }
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          ...
          <div className="flex justify-between items-center mt-8">
            <button
              ...
              className="flex items-center px-4 py-2 bg-red-800 text-white rounded-lg hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
        ...
        <div className="relative max-w-6xl mx-auto px-4 py-16">
          ...
          <div className="text-center">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 max-w-2xl mx-auto">
              <div className="text-center">
                <Calendar className="mx-auto mb-2 text-yellow-300" size={24} />
                <div className="text-lg font-bold">{book.sessions}</div>
                <div className="text-sm text-gray-300">Sessions</div>
              </div>
              <div className="text-center">
                <BookOpen className="mx-auto mb-2 text-yellow-300" size={24} />
              ...
              <div className="text-center">
                <MapPin className="mx-auto mb-2 text-yellow-300" size={24} />
      ...
      <section className="max-w-6xl mx-auto px-4 py-16">
        ...
        {entries.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-gray-500">Check back soon for exciting adventure logs!</p>
          </div>
        ) : (
