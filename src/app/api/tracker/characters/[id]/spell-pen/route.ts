// List + create spell penetration entries on a character.
...
export async function GET(_req: NextRequest, ctx: Ctx) {
  ...
  return json({ spell_pens: (data ?? []) as SpellPenEntry[] })
}
...
export async function POST(req: NextRequest, ctx: Ctx) {
  ...
  let body: CreateSpellPenBody
  try {
    body = (await req.json()) as CreateSpellPenBody
  } catch {
    return bad('invalid JSON body')
  }

  if (!body.name?.trim()) return bad('name is required')
  if (!Number.isFinite(body.bonus)) return bad('bonus must be a number')
  ...
  return json({ spell_pen: data as SpellPenEntry }, { status: 201 })
}
